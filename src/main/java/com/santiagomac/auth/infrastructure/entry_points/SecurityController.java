...
public class SecurityController {
  ...
  public ResponseEntity<Void> login(
    ...
    log.info("Login session for user: {}", authRequest.getEmail());
  ...
  private ResponseEntity<Void> setCookies(HttpServletResponse response, AuthResponse session) {
    ResponseCookie accessCookie =
        ResponseCookie.from("access_token", session.getAccessToken())
            .httpOnly(true)
            .secure(true)
            .sameSite("Lax")
            .path("/")
    ...
    ResponseCookie refreshCookie =
        ResponseCookie.from("refresh_token", session.getRefreshToken())
            .httpOnly(true)
            .secure(true)
            .sameSite("Lax")
            .path("/api/v1/auth")
